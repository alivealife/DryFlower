<template>
  <div>
    ...
    <div class="cart-dropdown">
      <form>
        <button class="btn btn-outline-secondary btn-cart" type="button" id="dropdownMenuButton" @click="openCart">
        ...
        <div class="cart-menu mb-3 p-2 bg-white">
          <table class="table">
            ...
            <tbody>
              ...
              <tr v-for="(item) in cartData.carts" :key="item.id" v-else>
                ...
                <td><img :src="item.product.imageUrl" style="height: 40px; width: 38px" class="img-thumbnail d-none d-md-inline-block"> 
                {{ item.product.title }}
                </td>
                <td width="80">
          ...
          <button class="btn btn-outline-main btn-lg btn-block rounded-pill" @click.prevent="checkout">結帳去</button>
        </div>
      </form>
    </div>
  </div>
