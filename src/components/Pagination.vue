    ...
    <nav aria-label="Page navigation example">
      <ul class="pagination justify-content-center">
        ...
        <li class="page-item" :class="{'disabled': !paginationData.has_pre}">
          ...
          <a class="page-link" href="#" aria-label="Previous" @click.prevent="changePage(paginationData.current_page - 1)">
            <span aria-hidden="true">&laquo;</span>
          </a>
        </li>      
                                                                          <!-- pagination.current_page == page 時就機 active 標籤 -->
        <li class="page-item" v-for="page in paginationData.total_pages" :key="page" :class="{'active': paginationData.current_page == page}">
        ...
        <li class="page-item" :class="{'disabled': !paginationData.has_next}">
                                                          <!-- 點擊時將目前分頁 pagination.current_page +1 代入 getProducts 達成切換分頁 -->
          <a class="page-link" href="#" aria-label="Next" @click.prevent="changePage(paginationData.current_page + 1)">
